 ...
 */
public final class PicassoMockRequestHandler extends RequestHandler {
    private final NetworkBehavior behavior;
    private final AssetManager assetManager;
    private static final int DISK_CACHE_SIZE = 50 * 1024 * 1024;

    /** Emulate the disk cache by storing the URLs in an LRU using its size as the value. */
    private final LruCache<String, Long> emulatedDiskCache = new LruCache<String, Long>(DISK_CACHE_SIZE) {
        @Override protected int sizeOf(String key, Long value) {
            return (int) Math.min(value, Integer.MAX_VALUE);
        }
    };

    public PicassoMockRequestHandler(NetworkBehavior behavior, AssetManager assetManager) {
